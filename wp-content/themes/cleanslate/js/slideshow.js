$j(document).ready(function() {
    ...
    $j.each(imagesArray, function(i) {
        ...
        $j('#pager').append('<a href="#" class="page" data-index="' + i + '" data-position="' + imgPosition + '">' + pagerNumber + '</a>');
        
        if(pagerNumber < imagesArray.length) {
            $j('#pager a').last().after('<span>&nbsp;|&nbsp;</span>')
        }
    ...
    // get pager width
    var pagerWidth = 0;
    $j('#pager').children().each(function() {
    ...
    $j('#media .image-container figure img').each(function(i) {
        
        var width = $j(this).width();
        var height = $j(this).height();
        var marginTop = (400 - height)/2 + 'px';
